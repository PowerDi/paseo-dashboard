...
import { useDashboardHostMutations } from "@/runtime/dashboard-host-mutations";
import {
  getHostRuntimeStore,
...
function ConnectionsSection({ host }: { host: HostProfile }) {
  const { t } = useTranslation();
  const { removeConnection } = useDashboardHostMutations();
...
function RemoveHostSection({
  ...
  const { t } = useTranslation();
  const { theme } = useUnistyles();
  const { removeHost } = useDashboardHostMutations();
