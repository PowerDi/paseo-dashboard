...
export function isDashboardHostRuntimeMode(): boolean {
  return process.env.EXPO_PUBLIC_PASEO_DASHBOARD === "1";
}
...
export class HostRuntimeStore {
  ...
  private readonly dashboardMode: boolean;
  ...
  constructor(input?: {
    ...
    this.dashboardMode = input?.dashboardMode ?? isDashboardHostRuntimeMode();
  }
  ...
  replaceDashboardAccountHosts(hosts: HostProfile[]): void {
    this.assertDashboardMode("replaceDashboardAccountHosts");
    this.hostRegistryStatus = "ready";
    this.hostRegistryLoaded = true;
    this.replicaCache.start();
  ...
  async clearDashboardAccountHosts(): Promise<void> {
    this.assertDashboardMode("clearDashboardAccountHosts");
    const controllerStops = this.setHostsAndSync([]);
    await Promise.all(controllerStops);
  }
  ...
  async upsertDirectConnection(input: {
    ...
    this.assertDirectConnectionsAllowed();
  ...
  private assertDashboardMode(method: string): void {
    if (!this.dashboardMode) {
      throw new Error(`${method} is only available in Dashboard mode.`);
    }
  }

  private assertDirectConnectionsAllowed(): void {
    if (this.dashboardMode) {
      throw new Error(DASHBOARD_DIRECT_CONNECTION_ERROR);
    }
  }
  ...
  private setHostsAndSync(
    ...
    this.hosts = hosts;
    const controllerStops = this.syncHostsInternal(hosts, options);
    this.emitHostList();
    return controllerStops;
  }
  ...
  syncHosts(
    ...
    void Promise.all(this.syncHostsInternal(hosts, options));
  }

  private syncHostsInternal(
    hosts: HostProfile[],
    options?: {
      initialConnectionByServerId?: Map<
        string,
    ...
    const controllerStops: Promise<void>[] = [];
