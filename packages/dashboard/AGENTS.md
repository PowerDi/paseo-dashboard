...
- 除非用户在当前对话中明确授权，Dashboard 开发绝不重启、停止、kill 或 reload 监听 `6767` 的 Paseo 主 daemon。
- 端口 `6767` 只允许只读检查状态和日志。超时或测试失败不能触发重启；Dashboard 的开发服务使用 `3000`、`5173` 或临时端口。
- 任何经明确授权的 `6767` 操作，执行前先说明会影响所有运行中的 agent，并确认目标进程确实是主 daemon。
